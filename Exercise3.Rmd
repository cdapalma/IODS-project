...
#url_por <- paste(por, "student-por.csv", sep = "/")
...
# Use glimpse() to look at the joined data and check data types
glimpse(math_por)
...
g3 <- ggplot(alc, aes(x = high_use, y=absences, fill=sex))
g3 =geom_bar()
```
...
# add 'sex' as another explanatory variable to the model
m_with_sex <- glm(high_use ~ failures + absences + sex, data = alc, family = "binomial")

# print out a summary of the updated model
summary(m_with_sex)

# print out the coefficients of the updated model
coef(m_with_sex)
...
alc <- mutate(alc, prediction = ifelse(probability > 0.5, "Yes", "No"))
...
# adjust the code: Perform 10-fold cross validation. Print out the mean prediction error for the testing data.
